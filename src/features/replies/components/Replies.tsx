...
import { Reply } from "src/features/replies/components/Reply";
import { useReplies } from "src/features/replies/hooks/useReplies";
        ...
        <Disclosure>
          <Transition as={Fragment} show={isOpen}>
            <Disclosure.Panel as="ul" className="mt-4 space-y-4" static>
              {data?.map((reply) => (
                <Reply key={reply.id} original_id={reply_id} reply={reply} />
              ))}
              {isLoading && (
