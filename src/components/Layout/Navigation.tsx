...
const navList = mockNavigation.map((item) => (
  <Link
    key={item.name}
    className="group relative flex items-center  justify-center rounded-md  px-2 py-1.5 font-hiragino-sans text-xs font-medium hover:bg-indigo-50 md:py-2 md:text-sm"
    href={`${item.href}`}
  >
